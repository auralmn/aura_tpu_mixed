...
from aura.utils.rope_jax import build_rope_cache, apply_rope
...
def generate_text(prompt_embeddings: jnp.ndarray,
                  ...
                  embed_fn: Optional[Callable[[jnp.ndarray], jnp.ndarray]] = None) -> Tuple[jnp.ndarray, List[jnp.ndarray]]:
    """
    ...
    # Prepare RoPE caches if enabled
    if use_rope:
        embed_dim = input_token.shape[-1]
        dim_even = embed_dim - (embed_dim % 2)
        rope_len = min(max_len + 1, rope_max_len)
        cos_cache, sin_cache = build_rope_cache(rope_len, dim_even, base=rope_base)
    generated_tokens = []
    all_rates = []
    ...
    if use_rope:
        pos = 0
        dim_even = input_token.shape[-1] - (input_token.shape[-1] % 2)
        rotated = apply_rope(input_token[..., :dim_even], cos_cache[pos], sin_cache[pos])
        input_rope = jnp.concatenate([rotated, input_token[..., dim_even:]], axis=-1)
        h_t = retrieval_core(input_rope)
    else:
    ...
    for t in range(max_len):
        ...
        if embed_fn is not None:
            input_token = embed_fn(next_token_idx)  # [batch, embed_dim]
        else:
            # Fallback: simple numeric repeat
            input_token = jnp.expand_dims(next_token_idx.astype(jnp.float32), -1)
            input_token = jnp.repeat(input_token, prompt_embeddings.shape[1], axis=-1)
        
        # 5) Retrieve context for next token
        if use_rope:
            pos = min(t + 1, cos_cache.shape[0] - 1)
            dim_even = input_token.shape[-1] - (input_token.shape[-1] % 2)
            rotated = apply_rope(input_token[..., :dim_even], cos_cache[pos], sin_cache[pos])
            input_rope = jnp.concatenate([rotated, input_token[..., dim_even:]], axis=-1)
            h_t = retrieval_core(input_rope)
